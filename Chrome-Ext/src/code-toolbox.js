// Code Toolbox Module - Manages saved code snippets and auto-execution
export class CodeToolbox {
  constructor() {
    this.tools = new Map(); // id -> tool data
    this.executionHistory = new Map(); // executionId -> {toolId, tabId, undoCode}
    this.autoRunRules = new Map(); // toolId -> {allPages: boolean, sites: Set<string>}
  ...
  async setAutoRun(toolId, enabled, scope = 'site') {
    ...
    if (!enabled) {
      ...
      const rule = this.autoRunRules.get(toolId) || { allPages: false, sites: new Set() };
  ...
  getAutoRunInfo(toolId) {
    ...
    if (!rule) return { enabled: false, allPages: false, sites: [] };
  ...
  async executeAutoRunTools(tab) {
    ...
    for (const [toolId, rule] of this.autoRunRules.entries()) {
      ...
      if (shouldRun) {
        // Delay execution to ensure page is ready
        setTimeout(() => {
  ...
  recordExecution(toolId, tabId, undoCode = null) {
    ...
    if (this.executionHistory.size > 50) {
      const sorted = Array.from(this.executionHistory.entries())
  ...
  getRecentExecutions(toolId = null) {
    const executions = Array.from(this.executionHistory.entries())
      .filter(([_, data]) => !toolId || data.toolId === toolId)
