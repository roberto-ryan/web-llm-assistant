  ...
  var ElementPicker = class {
    ...
    extractElementData(element) {
      ...
      const data = {
        ...
        html: element.outerHTML.length > 500 ? element.outerHTML.slice(0, 500) + "..." : element.outerHTML,
    ...
    getOptimalSelector(element) {
      ...
      if (element.className && typeof element.className === "string") {
        const classes = element.className.trim().split(/\s+/).filter((c) => c && !c.match(/^(active|hover|focus|disabled)$/i)).map((c) => `.${CSS.escape(c)}`).join("");
